...
def predict(loader, model, zeroshot_weights, softmax_eval=True, verbose=0): 
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(loader)):
            ...
            if logits.size(0) == 1:
                logits = np.squeeze(logits.cpu().numpy(), axis=0)  # Only squeeze if first dimension is 1
            else:
                logits = logits.cpu().numpy()  # No squeezing if the size of the first dimension is not 1
